...
void user_Preconditioner_function_void(Ax_struct *structAx, real *vec_f_in, real *vec_f_out){
    int N = structAx->N;
    dim3 threads(BLOCKSIZE);
    int blocks_x=(N+BLOCKSIZE)/BLOCKSIZE;
    dim3 blocks(blocks_x);
    
    copy_vectors<<< blocks, threads>>>(N, vec_f_in, vec_f_out);
    
}
void user_Preconditioner_function(Ax_struct_1 *SC, real *vec_f_in, real *vec_f_out){
    Arnoldi::matrixMultVector_GPU(SC->handle, SC->N, SC->A_d, SC->N, 1.0, vec_f_in, 0.0, vec_f_out);
}
